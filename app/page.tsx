...
function HomeGame() {
  ...
  const world = () => {
    ...
    if (scene === 1) return <div className={`scroll-world world art-world ${workshopIntro ? `arrival-${workshopIntro}` : "arrival-complete"} ${interactionId ? `interaction-${interactionId}` : ""}`}><div className="workshop-pan" /><div className="rain-lines" />{workshopIntro === "haul" && <div className="jade-arrival-sequence" role="img" aria-label="远景中，众人将玉料从舟船搬运到工坊码头">{Array.from({ length: 5 }).map((_, index) => <img key={index} className={`arrival-frame fixed-frame frame-${index + 1}`} src={`assets/arrival_new_${String(index + 1).padStart(2, "0")}.webp`} alt="" />)}<div className="arrival-film-caption"><span>舟船靠岸</span><i /><span>合力移玉</span></div></div>}{play}</div>;
